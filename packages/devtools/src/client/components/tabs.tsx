	...
	return (
		<div
			...
			class={clsx(
				...
				props.activeTab !== props.tab.id && "hover:bg-[#212121]",
				props.activeTab === props.tab.id && "bg-[#212121]",
				"hover:bg-[#212121]/50"
			)}
		>
			<div class={clsx(props.class, "group-hover:opacity-80 transition-all")}>{props.tab.icon}</div>
			<div
				class={clsx(
				...
				<For each={tabs}>
					{(tab) => (
						<Tab
							...
							class={clsx("cursor-pointer")}
						/>
					)}
				</For>
				<div class={clsx("mt-auto flex w-full flex-col items-center")}>
					<Tab
